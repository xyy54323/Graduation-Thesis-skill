...
制图前读取 [`references_diagram/图片制作规则.md`](references_diagram/图片制作规则.md)，再读取 `reference-images/` 对应图类型目录下的 `制作图片规则.md`，并打开该目录 PNG 示例图，必要时再打开同名 `.drawio` 查看线条、配色、字体和布局；再根据当前项目证据制作图片。如果没有完全对应类型，选择最接近的示例和制作规则，并在操作报告中说明。
...
- 所有图片线条不得穿过节点、文字或箭头，不得交叉覆盖；发现后必须调整布局、增加折点、拆图或返工。
...
4. 回写文字、表格、图片和图题。
5. 回写完成后按 `论文格式.md` 逐项检查目录、页眉页脚、页码域、分页符、交叉引用、图表编号、三线表线宽、页面留白、图片大小和正文格式；未检查不得交付。
...
- 降 AIGC 导出 Word 后，必须立即逐项恢复并验证正文参考文献引用跳转；如果引用编号变成普通文本，先恢复引用域或超链接，再写说明文件或进入交付检查，未恢复不得交付。
...
- `论文格式.md` 已经用户确认；未确认不得最终交付。
- 目标 Word 已按分类规则备份，用户手改稿没有被误覆盖。
- 封面、声明等学校固定页面未被误改；每章后面、中文摘要、英文摘要有分页符；页面下方无不合理大面积留白；图片大小已适当调节且未变形。
- 自动目录、页眉页脚、页码、交叉引用和参考文献跳转未被破坏；执行过降 AIGC 后，必须已经逐项恢复并验证正文参考文献引用跳转，不能只记录问题。
