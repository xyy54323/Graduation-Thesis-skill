...
### Special case: Chinese thesis functional structure diagrams

When the user asks for `功能结构图`, `系统功能结构图`, `功能结构树`, or wants a graduation-thesis-style functional hierarchy diagram, default to the black-and-white organizational-tree layout described in `references/functional-structure-cn.md`.

In this case:
- Read `references/functional-structure-cn.md` before generating.
- Use the bundled visual reference `assets/functional-structure-cn-example.png` as the canonical example.
- Prefer true vertical labels for second-level function boxes, with one character or letter per line.
- Keep first-level modules horizontal and second-level modules vertical.
- Use independent orthogonal branch lines per first-level group and avoid connector overlap.
- If the user provides another reference image, follow the user's image over the bundled example.

### Special case: Chinese thesis flowcharts

When the user asks for `流程图`, `软件流程图`, `程序流程图`, or wants a graduation-thesis-style Chinese flowchart, default to the strict black-and-white thesis flowchart rules described in `references/flowchart-cn.md`.

In this case:
- Read `references/flowchart-cn.md` before generating.
- Use the bundled visual reference `assets/flowchart-cn-example.png` as the canonical example.
- Always use terminator nodes for `开始` and `结束`.
- Keep same-type node box sizes consistent.
- Use orthogonal connectors only; connectors must not cross, pass through nodes, or overlap unrelated node borders.
- Avoid placeholder loop nodes unless the user explicitly requires them; connect branch loops back to meaningful workflow nodes.
- Do not place an internal decorative title at the top of the figure unless the user explicitly asks for one.
- If the user provides another reference image, follow the user's image over the bundled example.
