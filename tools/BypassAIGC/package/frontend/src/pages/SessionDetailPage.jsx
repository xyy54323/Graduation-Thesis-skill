...
const HISTORY_ROUTE = '/?view=history';
...
const SessionDetailPage = () => {
  ...
  const loadSessionDetail = async () => {
    try {
      ...
      setExportFormat((current) => {
        if (!sessionData.has_source_doc && current === 'docx') {
          return 'txt';
        }
        return current;
      });
  ...
  const handleExport = async (acknowledged) => {
    ...
    try {
      ...
      const fallbackName = exportFormat === 'docx' ? `optimized_${sessionId}.docx` : `optimized_${sessionId}.txt`;
      const blob = new Blob([response.data], {
        type: response.headers?.['content-type'] || (exportFormat === 'docx' ? DOCX_MEDIA_TYPE : 'text/plain;charset=utf-8'),
      });
      const url = window.URL.createObjectURL(blob);
      const a = document.createElement('a');
      a.href = url;
      a.download = getDownloadFilenameFromHeaders(response.headers, fallbackName);
      a.click();
      ...
      const message = await readBlobErrorMessage(error.response?.data, '导出失败');
      toast.error(message);
    }
  };
          ...
          <div className="bg-gray-200/80 p-1 rounded-xl inline-flex w-full max-w-md">
            <button
              ...
              className={`flex-1 py-1.5 px-4 rounded-[9px] text-[13px] font-medium transition-all duration-200 ${
                activeTab === 'result' ? 'bg-white text-black shadow-sm' : 'text-gray-600 hover:text-black'
              }`}
            >
            ...
            <button
              ...
              className={`flex-1 py-1.5 px-4 rounded-[9px] text-[13px] font-medium transition-all duration-200 ${
                activeTab === 'compare' ? 'bg-white text-black shadow-sm' : 'text-gray-600 hover:text-black'
              }`}
            >
              <div className="flex items-center justify-center gap-2">
        ...
        <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                  ...
                  <div className="flex items-center gap-3">
                    <h3 className="text-[15px] font-semibold text-black ml-2">
                      {shouldShowResultSwitch()
                        ? (resultViewMode === 'enhanced' ? '降AI后的文本' : '润色后的文本')
                        : '优化后的文本'}
                    </h3>
                      ...
                      <div className="bg-gray-200/80 p-0.5 rounded-lg inline-flex">
                        <button
                          ...
                          className={`py-1 px-3 rounded-md text-[12px] font-medium transition-all ${
                            resultViewMode === 'polished' ? 'bg-white text-black shadow-sm' : 'text-gray-600 hover:text-black'
                          }`}
                        >
                          润色
                        </button>
                        <button
                          ...
                          className={`py-1 px-3 rounded-md text-[12px] font-medium transition-all ${
                            resultViewMode === 'enhanced' ? 'bg-white text-black shadow-sm' : 'text-gray-600 hover:text-black'
                          }`}
                        >
                          降AI
                        </button>
              ...
              <div className="bg-white rounded-2xl shadow-ios overflow-hidden flex flex-col h-[calc(100vh-180px)]">
                <div className="p-3 bg-gray-50 border-b border-gray-100">
                  <h3 className="text-[15px] font-semibold text-gray-500 ml-2">原始文本</h3>
                </div>
          ...
          {activeTab === 'compare' && (
            <div className="bg-white rounded-2xl shadow-ios p-6 min-h-[calc(100vh-180px)]">
              <h3 className="text-[20px] font-bold text-black mb-6 tracking-tight">变更对照记录</h3>
              ...
              {changes.length === 0 ? (
                <div className="text-center py-12">
                  ...
                  <p className="text-ios-gray">暂无变更记录</p>
                </div>
              ) : (
                <div className="space-y-6">
                  {changes.map((change) => (
                    <div key={change.id} className="border border-gray-100 rounded-xl p-5 hover:shadow-md transition-shadow">
                      <div className="flex items-center gap-2 mb-4">
                        ...
                        <span className="bg-blue-50 text-ios-blue text-[11px] font-bold px-2 py-1 rounded-md uppercase tracking-wide">
                          {change.stage === 'polish' ? '润色' : '降AI'}
                        </span>
                      </div>

                      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                          <h4 className="text-[13px] font-semibold text-ios-gray mb-2 uppercase tracking-wide">修改前</h4>
                        ...
                        <div>
                          <h4 className="text-[13px] font-semibold text-ios-gray mb-2 uppercase tracking-wide">修改后</h4>
        ...
        <div className="fixed inset-0 bg-black/30 backdrop-blur-sm flex items-center justify-center p-4 z-[100]">
          <div className="bg-white rounded-[14px] shadow-2xl max-w-sm w-full overflow-hidden animate-in fade-in zoom-in duration-200">
            <div className="p-6 text-center">
              ...
              <h2 className="text-[17px] font-semibold text-black mb-2">学术诚信确认</h2>
              <p className="text-[13px] text-black mb-4">请确认您已审核所有内容，并对最终论文负责。</p>
              ...
              <div className="mb-4">
                <label className="block text-[12px] font-medium text-ios-gray mb-1.5 text-left">导出格式</label>
                <select
                  value={exportFormat}
                ...
                >
                  ...
                  <option value="docx" disabled={!session.has_source_doc}>
                    {session.has_source_doc ? 'Word文档 (.docx)' : 'Word文档 (.docx) - 仅 Word 上传任务支持'}
                  </option>
                  <option value="pdf" disabled>PDF文件 (.pdf) - 即将支持</option>
                </select>
                {session.has_source_doc && (
                  <p className="mt-2 text-[12px] text-ios-gray text-left">
                    当前会话来自 Word 正文提取，导出 `.docx` 时会把优化后的正文回写到原论文版式中。
                  </p>
                )}
              </div>
            </div>
